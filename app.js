...
const fmt=p=>`${(Number(p)*100).toLocaleString('fi-FI',{minimumFractionDigits:1,maximumFractionDigits:1})} %`;
...
function renderRatings(data){const body=document.querySelector('#ratingsBody');if(!body||!Array.isArray(data.ratings))return;document.querySelector('#ratingsUpdated').textContent=`Päivitetty ${new Date(data.as_of).toLocaleString('fi-FI',{dateStyle:'medium',timeStyle:'short'})}. Kauden ${data.season} ottelut vaikuttavat arvioihin heti datan valmistuttua.`;body.innerHTML=data.ratings.map((row,index)=>`<tr><td>${index+1}</td><td>${esc(teamNames[row.team]||row.team)}</td><td>${Number(row.offense).toFixed(3)}</td><td>${Number(row.defense).toFixed(3)}</td><td>${Number(row.qb).toFixed(3)}</td><td><strong>${Number(row.overall).toFixed(3)}</strong></td></tr>`).join('')}
fetch('./public/data/team-ratings.json',{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject()).then(renderRatings).catch(()=>{});
